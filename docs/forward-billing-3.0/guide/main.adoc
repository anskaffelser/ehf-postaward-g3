...
| BR-07
| Element cac:AccountingCustomerParty/cac:Party/cac:PartyLegalEntity is mandatory in EN 16931. This rule is removed to make cac:PartyLegalEntity optional. In most sectors, the presence of the element indicates a business customer and its absence indicates a consumer customer. For the energy sector and "gjennomfakturering", cac:PartyLegalEntity is instead used to identify the power supplier (see chapter 3.1.5).

| PEPPOL-EN16931-R007
| Process identifier used in the forward billing transactions belongs to DFØ. This rules is removed to deviate the process identifier.
|===
...
The presence or absence of cac:PartyLegalEntity is used to distinguish between business and consumer customers. This differentiation does not apply to the energy sector when used for "gjennomfakturering", where cac:PartyLegalEntity instead identifies the power supplier. See chapter 3.1.5.
